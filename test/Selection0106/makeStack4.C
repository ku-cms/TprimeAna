...
void DrawPlot( TCanvas * c1, TH1D * hBack, TH1D * hData, std::map<TH1D*,TString> &hSignal, TH1D * hQCD = 0, TH1D * histW = 0, TH1D * histTT = 0 )
{
	...
	TString lumiText = "1.63 fb^{-1} (13 TeV)";
	...
	if(hBack) {
	//	hBack->DrawCopy("hist"); 
		hBack->SetMarkerStyle(0);
		hBack->SetFillColor(kGray+2);
  		hBack->SetFillStyle(3018);
  		hBack->Draw("e2same");
  	        hBack->Draw("L same");
	}
	...
	if(hBack) leg->AddEntry(hBack,"Stat.+x-sec unc.");
...
double makeStack(TString plot, TString cuts, TString Signal, TString label,TString yLabel  ,TString title, int sigScale ,int rebin , int bins, int xMin, int xMax, TString sScale, bool bSep, bool bData);

/*
double makeStackEta(TString plot, TString cuts,TString label,TString title,  int rebin =1, int yMax = 2000, bool sym = true) 
{
	double srb ;

	if(sym)

		srb = makeStack(plot,cuts,label,title, rebin,100,0,5,yMax,false);

	else
		srb = makeStack(plot,cuts,label, title, rebin, 200, -5 , 5, yMax,false);
...
double makeStack(TString plot,TString cuts, TString Signal, TString label, TString yLabel, TString title,int sigScale = 1, int rebin = 1,int bins=1000, int xMin=0, int xMax=1000, TString sScale="1",bool bSep = false, bool bData = true)
{
	...
	TChain * tree2 = new TChain("analyze/cutTree");
	TChain * tree6 = new TChain("analyze/cutTree");
	TChain * tree7 = new TChain("analyze/cutTree");
	TChain * tree8 = new TChain("analyze/cutTree");
	TChain * tree9 = new TChain("analyze/cutTree");
	TChain * tree10 = new TChain("analyze/cutTree");

	TChain * treeData = new TChain("analyze/cutTree");

	treeData->Add("JetHT2015D-2000*.root");

	std::map<TFile* , TString> fSig ;
	...
	TObjArray * sigNames = Signal.Tokenize(TString(";"));
		...
		if (s != "")
			fSig.insert(make_pair(new TFile (s+".root","READ"),s));
	}
        //TFile * f1 = new TFile ("Tprime1200_LHv3.root","READ");

        tree2->Add("TTJets*.root");
	...
	cout << "QCD500: " << tree6->Add("QCDHT500v2*.root") << endl;
        cout << "QCD700: " << tree7->Add("QCDHT700v2*.root") << endl ;
        cout << "QCD1000: " << tree8->Add("QCDHT1000v2*.root") << endl;
        cout << "QCD1500: " << tree9->Add("QCDHT1500v2*.root") << endl;
        cout << "QCD2000: " << tree10->Add("QCDHT2000v2*.root") << endl;
        TFile * f11 = new TFile ("WJetsv2.root","READ");
...
*/
	...
	treeData->Draw(plot+">>htdata",cuts+"&&bTrigger");
	...
	double lumi = 1.63 / 2; //1.929/2 ;//1.28/2;
	...
	for( std::map<TFile * , TString>::iterator it = fSig.begin() ; it != fSig.end(); it++) {
	    ...
	    hit->first->Scale(0.02*lumi*sigScale);			
            hit->first->SetLineColor(kRed+c);
	    c++;
	}

	//hist1 = (TH1D*) gROOT->FindObject("ht1");
	hist2 = (TH1D*) gROOT->FindObject("ht2");
	...
	hist11 = (TH1D*) gROOT->FindObject("ht11");
	histData = (TH1D*) gROOT->FindObject("htdata");

	//hist1->Scale(0.00442/3);	
	//hist1->Scale(.02*lumi*sigScale); // 2fb-1 @ 20pb
        hist2->Scale(0.039*lumi);
        hist3->Scale(672*lumi);
        hist4->Scale(186*lumi);
        hist5->Scale(36*lumi);
        hist6->Scale(3*lumi);
        hist7->Scale(0.836*lumi);
        hist8->Scale(0.428*lumi);
        hist9->Scale(0.061*lumi);
        hist10->Scale(0.026*lumi);
        hist11->Scale(0.218*lumi);
...
//	stack->GetXaxis()->SetTitle(label);
	...
	//   cout << "Bin: " << i << "  Stat: " << fromStack->GetBinError(i) << " cont: " << fromStack->GetBinContent(i) << endl; 
...
void Closure(Int_t H_Sj, Int_t TTag)
{
	TString out,cut;
	out.Form("mass(ptAK8[HTagIdxSB%d[0]],etaAK8[HTagIdxSB%d[0]],phiAK8[HTagIdxSB%d[0]],MAK8[HTagIdxSB%d[0]], ptAK8[TTagIdxSB[0]],etaAK8[TTagIdxSB[0]],phiAK8[TTagIdxSB[0]],MAK8[TTagIdxSB[0]])",H_Sj,H_Sj,H_Sj,H_Sj);

	cut.Form("htak4>900&&@HTagIdxSB%d[0].size()>0&&@TTagIdxSB[0].size()>0&&fMaxEtaAK4<2.0",H_Sj);
	makeStack(out,cut,"T' (GeV)", "SideBand" ,100,1000,0,2000,100);

	TH1D * hBackEst = (TH1D*) gROOT->FindObject("hBack");

	if(!hBackEst) {
		cout << "Can't find Background plot" << endl;
		return;
	}

//	TH1D * hBackEst = (TH1D*)((TH1D*)(histSB->GetStack()->Last()))->Clone();
	
	
	out.Form("mass(ptAK8[HTagIdx[0]],etaAK8[HTagIdx[0]],phiAK8[HTagIdx[0]],MAK8[HTagIdx[0]], ptAK8[TTagIdx%02d[0]],etaAK8[TTagIdx%02d[0]],phiAK8[TTagIdx%02d[0]],MAK8[TTagIdx%02d[0]])",TTag,TTag,TTag,TTag);
        cut.Form("htak4>900&&@HTagIdx[0].size()>0&&@TTagIdx%02d[0].size()>0&&fMaxEtaAK4>2.0",TTag);
        makeStack(out,cut,"T' (GeV)","Signal Region",100,1000,0,2000,100);

	
	TH1D * histData = (TH1D*) gROOT->FindObject("htdata");

	if(!histData) {
		cout << "Can't find data plot" << endl;
		return;
	}
	
	double SF = histData->Integral(0,1000)/hBackEst->Integral(0,1000);
	cout << "Scale Factor: " <<SF << endl;
	...
	//DrawPlot( c1 , hBackEst , histData );


}

/*
void MCPlot(Int_t H_Sj, Int_t TTag)
{
        TString out,cut;
        out.Form("mass(ptAK8[HTagIdxSB%d[0]],etaAK8[HTagIdxSB%d[0]],phiAK8[HTagIdxSB%d[0]],MAK8[HTagIdxSB%d[0]], ptAK8[TTagIdxSB[0]],etaAK8[TTagIdxSB[0]],phiAK8[TTagIdxSB[0]],MAK8[TTagIdxSB[0]])",H_Sj,H_Sj,H_Sj,H_Sj);
	...
	cout << "processed data" << endl; 
        ...
        TH1D * histQCD = (TH1D* )((TH1D*) gROOT->FindObject("histQCD"))->Clone();
        TH1D * histW = (TH1D*)((TH1D*) gROOT->FindObject("ht11"))->Clone();
        TH1D * histTT = (TH1D*) ((TH1D*) gROOT->FindObject("ht2"))->Clone();

        TH1D * histData = (TH1D*) gROOT->FindObject("hBack");
	
	cout << histQCD << histW << histTT << histData << endl;
        if(!histData) {
                cout << "Can't find data plot" << endl;
                return;
        }
        ...
        DrawPlot( c1 , hBackEst , histData ,0, histQCD, histW, histTT);
...
void ABCDData(bool bMt)//TString plot, TString cutA, TString cutB, TString cutC, TString cutD)
{
	makeStack("htak4", "htak4>1000&&@HTagIdxSB0.size()>0&&@TTagIdx03.size()==0" ,"Tprime1200_LH","HT GeV","Events","Cut A",1,1,20,1000,2600,"EvtWeight[0]*EvtWeight[4]/1.21*WeightHT(htak4)");
	...
	if(bMt) 
		makeStack("mass(ptAK8[HTagIdxSB0[0]],etaAK8[HTagIdxSB0[0]],phiAK8[HTagIdxSB0[0]],MAK8[HTagIdxSB0[0]],ptAK8[TTagIdx03[0]],etaAK8[TTagIdx03[0]],phiAK8[TTagIdx03[0]],MAK8[TTagIdx03[0]])", "htak4>1000&&@HTagIdxSB0.size()>0&&@TTagIdx03.size()>0","Tprime1200_LH","HT GeV","Events","Cut B",1,1,36,600,2400,"EvtWeight[0]*EvtWeight[4]/1.21*WeightHT(htak4)");
	else
        	makeStack("htak4", "htak4>1000&&@HTagIdxSB0.size()>0&&@TTagIdx03.size()>0","Tprime1200_LH","HT GeV","Events","Cut B",1,1,20,1000,2600,"EvtWeight[0]*EvtWeight[4]/1.21*WeightHT(htak4)");
	...
	TH1D * histBW = (TH1D*) gROOT->FindObject("ht11")->Clone();
        ...
        makeStack("htak4", "htak4>1000&&@HTagIdx.size()>0&&@TTagIdx03.size()==0","Tprime1200_LH","HT GeV","Events","Cut C",1,1,20,1000,2600,"EvtWeight[0]*EvtWeight[4]/1.21*WeightHT(htak4)");
	...
	TH1D * histCW = (TH1D*) gROOT->FindObject("ht11")->Clone();
        ...
        if (bMt) makeStack("mass(ptAK8[HTagIdx[0]],etaAK8[HTagIdx[0]],phiAK8[HTagIdx[0]],MAK8[HTagIdx[0]],ptAK8[TTagIdx03[0]],etaAK8[TTagIdx03[0]],phiAK8[TTagIdx03[0]],MAK8[TTagIdx03[0]])", "htak4>1000&&@HTagIdx.size()>0&&@TTagIdx03.size()>0","Tprime1200_LH","HT GeV","Events","Cut D",1,1,36,600,2400,"EvtWeight[0]*EvtWeight[4]/1.21*WeightHT(htak4)");
	else
        	makeStack("htak4", "htak4>1000&&@HTagIdx.size()>0&&@TTagIdx03.size()>0","Tprime1200_LH","HT GeV","Events","Cut D",1,1,20,1000,2600,"EvtWeight[0]*EvtWeight[4]/1.21*WeightHT(htak4)");
...
//
        ...
        leg->AddEntry(histShapeBMC,"Sideband B");
